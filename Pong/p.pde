...
void Pong() {
  ...
  // Recta de la raqueta 2
   rectMode(CENTER);  // Set rectMode to CENTER
  rect(bolaPosx, height-450, raqLargo2, raqAncho2);
  ...
  if (gameStart) {
    ...
    if ( bolaPosy > raqPosy-radio && bolaPosy < raqPosy+radio) {
     ...
     // Si pelota toca la raqueta 2, cambiar dirección de Y
         if (bolaPosx > raqPosx2-raqLargo2/2-radio && bolaPosx < raqPosx2+raqLargo2/2 +radio) {

          if ( bolaPosy < raqPosy2+radio+raqAncho2){
        vely = vely * -1;
        puntos=puntos+1;
